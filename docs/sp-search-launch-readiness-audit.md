...
5. **Multi-context admin documentation page with worked example**
   ...
   - **Why it matters:** Ties to (a) T3 differentiator. The architecture is the moat versus PnP v4 (Appendix C "Multi-instance isolation via `searchContextId`" — Better grade); shipping zero documentation for the moat means the moat doesn't exist for any prospect who reads the docs before installing.
   - **Effort:** L
   - **Priority:** P1
   ...
   - **Acceptance signal:** `docs/multi-context-guide.md` exists and is linked from the top-level README and from `docs/admin-guide.md:9`. Screenshot count: ≥4 (one per web part property pane on the worked example, one of the resulting two-context URL). Troubleshooting flowchart covers the four named failure modes with named diagnostic checks (DevTools console output for alias collision, MessageBar text for mismatch warning, etc.).

6. **Per-context URL prefix opt-out and explicit-prefix override property**
   - **Description:** Today the URL prefix is auto-computed from `searchContextId` and only activates when `getContextMap().size > 1` (`storeRegistry.ts:176`). Add an admin-facing override: extend `IPropertyPaneSearchContextIdOptions` (the helper introduced in D4) with an optional `urlPrefix` property pane field (e.g. `?ctx1.q=...` instead of `?defautd87fe2.q=...`). When provided, the explicit prefix wins over `_buildStableUrlPrefix(searchContextId)` (`storeRegistry.ts:217, 292-297`). Also expose `enableUrlSync: boolean` (default `true`) so an admin can opt a context out of URL sync entirely (useful for embedded "saved-search runner" widgets that must not stomp the page URL). Wire both into the existing `createUrlSyncSubscription` factory (`urlSyncMiddleware.ts:665-783`) — when `enableUrlSync === false`, return the existing SSR no-op unsubscribe path.
   - **Why it matters:** Ties to (a) T3 differentiator. Required-Scenario S3 (URL deep-link parameters for two contexts on the same page) ships today with auto-computed 6+6-char prefixes that produce ugly URLs (`?defautd87fe2.q=annual+report&peopl4f3a91.q=john`). Admins running side-by-side contexts want short readable prefixes (`?ctx1.q=...&ctx2.q=...`), and embedders running headless contexts want URL-sync-off entirely. Both are quality-of-life improvements that compound with D5's documentation walkthrough.
   - **Effort:** M
   - **Priority:** P1
...
7. **Per-vertical `dataProviderId` validation and admin-time discoverability**
   - **Description:** `IVerticalDefinition.dataProviderId` is a free-text string today (`IVerticalDefinition.ts:16`, `SpSearchVerticalsWebPart.ts:125, 156`) with no validation against the registered providers. Add (a) a property-pane Dropdown (instead of free text) populated from `state.registries.dataProviders.getAll().map(p => ({key: p.id, text: p.id}))` so admins pick from the registered list; (b) edit-mode warning in `SpSearchVerticals.tsx` when a vertical references a `dataProviderId` not present in the registry (e.g. typo `'graph'` vs `'GraphSearchProvider'`), since the orchestrator's silent fallback (`SearchOrchestrator.ts:550-557`) currently masks the misconfiguration; (c) document each shipped provider's `id` in `docs/extensibility-guide.md` ("SharePoint Search provider id: `SharePointSearchProvider`. Graph provider id: `GraphSearchProvider`. Custom providers must register with a unique id.").
...
8. **`hasStore` / `getStore` audit panel in DebugFab for runtime context inspection**
   ...
   - **Effort:** L
   - **Priority:** P1
   - **Depends on:** Deliverable 2 (consumes `recordWebPartContextId` registry); T5 plan owns the broader DebugPanel composition — coordinate sequencing.
...
10. **Initialization-order diagnostic (close ARCH-003 against T3 surface)**
    ...
    - **Source:** Appendix A ARCH-003 (Still-Open); Journey B Step 11 [Polish] (URL filters silently lost on slow tenant); Phase 6.3 discovery: `storeRegistry.ts:191-198` documents the convention but provides no programmatic check
