...
| BUG-009 | Scope round-trip loses `kqlPath` and `resultSourceId` | High | Closed | Fix commit `6f226f2` first added base64 scope JSON encoding; subsequent simplification (commit `e199c80` "URL cleanup") removed scope from URL serialization entirely (no `PARAM_SCOPE` in `urlSyncMiddleware.ts`). Scope is now persisted to `localStorage` per `SpSearchBox.tsx:112,144-161,428-432` (cf. closed MISS-005). The original lossy round-trip is impossible. | T3 |
...
| INC-007 | Admin Manager is a re-export stub | High | Changed-Form | Fix commit `02c6adc` made it a standalone subclass (`SpSearchAdminManagerWebPart.ts:16` `extends SpSearchManagerWebPart`) with its own `manifest.json` + `onInit` (instruments DebugCollector). **Residual gap:** Behavior is still 100% inherited from the user Manager — preconfigured manifest defaults are the only differentiation; no admin-only code path beyond the variant flag. | T4 |
