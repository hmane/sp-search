...
`scripts/check-bundle-sizes.js` runs after `npm run package` (`heft build --production` + `gulp package-solution --ship`), discovers `release/assets/sp-search-*-web-part_<hash>.js` (strict — hashed bundles only) via regex, compares against `config/bundle-budgets.json`, and exits non-zero on breach (or on missing/ambiguous matches, including unhashed `npm run package:debug` output). CI wiring lives in `.github/workflows/build.yml` (Found.D8).
