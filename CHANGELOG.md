...
- Configure refiners surface redesigned as a master-detail Panel. `PropertyFieldCollectionData` (~21 columns shown for every row regardless of filter type) replaced with a custom `PropertyPaneFiltersCollection` control. The new control opens a 960px Panel with refiner list on the left and a sectioned form on the right; sections (Basic / Display / Behavior / Toggle labels / Conditional visibility / Audience) and individual fields are conditionally rendered based on the selected refiner's filterType — e.g. `trueLabel` / `falseLabel` / `invertBoolean` only appear for toggle; `sortBy` / `sortDirection` are hidden for people (person-ranked, not sortable); `multiValues` is hidden for dropdown. `IFiltersCollectionItem` shape is JSON-identical to the prior `IFilterCollectionItem`, so saved configs round-trip without migration. Pure visibility logic lives in `src/propertyPaneControls/filtersCollection/fieldRelevance.ts` with 13 unit tests.
- **Admin Manager: Coverage tab + dead properties removed.** The Coverage `PivotItem` rendered a single MessageBar pointing users to the Dashboard tab (its per-list/per-library inspection feature was never built). Tab removed entirely. With it: `enableCoverage` property (toggle for the dead tab) and `coverageSourcePageUrl` property (declared, validated, projected into props, but never read by any UI surface) are both gone from `ISpSearchManagerWebPartProps`, `ISpSearchManagerProps`, the admin property pane, the SpSearchAdminManager subclass projection, and `SpSearchAdminManagerWebPart.manifest.json`. The `defaultTab` ChoiceGroup now offers Dashboard / Health / Insights (Coverage removed; Dashboard added — it was missing despite being the manifest default). Admin Manager surface is now Health + Insights + Dashboard + Pre-Flight. `validateCoverageSourcePageUrl` validator + its 11 tests are retained as a generic SharePoint-URL validator for future fields.
